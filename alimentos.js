...
function searchMainDatabase() {
    ...
    if (query.length < 2) {
        list.innerHTML = `<div class="empty-state-search"><span style="font-size: 48px; display: block; text-align: center; margin-top: 40px;">🥗</span><p class="placeholder-text" style="margin-top: 16px;">Consulte calorias, macros, sódio e muito mais em nosso banco de dados.</p></div>`;
        return;
    }
    ...
    if (results.length === 0) {
        list.innerHTML = '<p class="placeholder-text" style="margin-top: 40px;">Nenhum alimento encontrado.</p>';
        return;
    }
...
function openFoodDetail(food) {
    ...
    content.innerHTML = `
        ...
        <div class="detail-macro-grid">
            <div class="dm-card"><span class="dm-val">${Math.round(food.c)}g</span><span class="dm-lbl">Carboidratos</span></div>
            <div class="dm-card"><span class="dm-val">${Math.round(food.p)}g</span><span class="dm-lbl">Proteínas</span></div>
            <div class="dm-card"><span class="dm-val">${Math.round(food.g)}g</span><span class="dm-lbl">Gorduras</span></div>
        </div>

        <div class="micro-list">
            <div class="micro-row"><span class="micro-lbl">Fibras Alimentares</span><span class="micro-val">${Math.round(fibrasMock)}g</span></div>
            <div class="micro-row"><span class="micro-lbl">Sódio</span><span class="micro-val">${Math.round(sodioMock)}mg</span></div>
        </div>
    `;
...
function renderRecentFoods() {
    ...
    if(consumedFoods.length === 0) {
        list.innerHTML = '<p class="placeholder-text" style="margin-top: 0;">Nenhum alimento registrado neste dia.</p>';
        return;
    }
